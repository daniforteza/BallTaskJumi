...
public class Channel implements Runnable {
    ...
    private Ball createBall(String ballInfo) {
        ...
        ball.setDimension(Integer.parseInt(info[1]));
        if(Integer.parseInt(info[4]) > this.ballTask.getHeight()){
            ball.setCordY(this.ballTask.getHeight() - ball.getDimension());
        }else{
            ball.setCordY(Integer.parseInt(info[4]));
        }
        if(this.ballTask.getDirection().equals("izquierda")){
            ball.setCordX(1 + ball.getDimension());
            ball.setVelX(1);
        }else if(this.ballTask.getDirection().equals("derecha")){
            ball.setCordX(this.ballTask.getViewer().getWidth() - (ball.getDimension() + 1));
            ball.setVelX(-1);
        }
        ...
        ball.setRect(new Rectangle(ball.getDimension(), ball.getDimension()));
        ball.getRect().setBounds(ball.getCordX(), ball.getCordY(), ball.getDimension(), ball.getDimension());
